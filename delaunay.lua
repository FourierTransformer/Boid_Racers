...
function Triangle:__init(p1, p2, p3)
  ...
  self.e = {Edge(p1, p2), Edge(p2, p3), Edge(p3, p1)}
end
...
function Triangle:getSidesLength()
  return self.e[1]:length(), self.e[2]:length(), self.e[3]:length()
end
  ...
  for i = 1, nvertices do
    ...
    for j = #triangles, 1, -1 do
      ...
      if curTriangle:inCircumCircle(vertices[i]) then
        edges[#edges + 1] = curTriangle.e[1]
        edges[#edges + 1] = curTriangle.e[2]
        edges[#edges + 1] = curTriangle.e[3]
        remove(triangles, j)
      end
    end
